...
struct uart_driver_interface
{
    ...
    uint32_t (*const write)(const uint8_t *, const uint16_t);  // Writes N bytes over UART - blocking
    uint32_t (*const read)(uint8_t *, const uint16_t);         // Reads N bytes over UART - blocking
};
